...
const (
	captureError  string = "🟥 Failed to capture."
	encodingError string = "🟥 Failed to encode screenshot."
)
...
func (*ScreenShotCommand) Run(s *discordgo.Session, m *discordgo.MessageCreate, args []string) error {
	...
	if err != nil {
		_, err := s.ChannelMessageSendReply(m.ChannelID, captureError, m.Reference())
		return err
	}
	...
	if err != nil {
		_, err := s.ChannelMessageSendReply(m.ChannelID, encodingError, m.Reference())
		return err
	}
